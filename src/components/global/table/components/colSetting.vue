<template>
    <a-popover placement="bottomLeft" trigger="click" @visible-change="handleVisibleChange">
        <template #content>
            <a-checkbox-group ref="columnListRef" v-model:value="checkedList" @change="setChecked">
                <template v-for="item in columns" :key="item.dataIndex">
                    <div class="w-[300px] flex items-center justify-between py-1">
                        ...
                        <div class="flex items-center">
                            <vertical-align-bottom-outlined class="rotate-90 !text-lg" :class="arrowClass(item, 'left')"
                                @click="setupFixed(item.dataIndex, 'left')" />
                            <a-divider type="vertical" />
                            <vertical-align-top-outlined class="rotate-90 !text-lg" :class="arrowClass(item, 'right')"
