<template>
  <div class="h-full">
    <w-table ref="table" :requestApi="roleList" :columns="tableColumns">
      ...
      <template #status="{ row }">
        <a-switch
          v-model:checked="row.record.status"
          @click="statusChange(row.record)"
          :checkedValue="1"
          checked-children="正常"
      ...
      >
        <template #menuIdFormItem>
          <a-checkbox-group v-model:value="roleBtnIds">
            <a-tree
              v-model:checkedKeys="rolePagesId"
              autoExpandParent
              defaultExpandAll
              show-line
              checkable
              ...
              <template #title="{ dataRef }">
                <div class="flex justify-between w-[400px]">
                  ...
                  <div class="w-[200px] text-right" @click.stop>
                    <a-checkbox
                      v-for="box in menuBtn.filter(x => x.pId === dataRef.id)"
                      :value="box.id"
                      :key="box.id"
                      @change="
                        change(
                          menuBtn.filter(x => x.pId === dataRef.id),
                          dataRef,
                        )
                      "
                    >
