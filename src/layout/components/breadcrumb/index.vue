<template>
  <a-breadcrumb separator="/" class="!ml-3" id="w-breadcrumb">
    <a-breadcrumb-item @click="router.push({ path: '/' })">
      <span class="font-bold text-black dark:!text-white cursor-pointer fz-md">
        {{ $t("commons.home") }}
      </span>
    </a-breadcrumb-item>
...
const { crumbsList } = inject<layoutInterface>("sysConfig");

const toPage = (value: menuListType) => {
  if (value.component !== "Layout")
    return router.push({ path: value.path || "" });
