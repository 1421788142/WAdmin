    ...
    <div class="flex flex-wrap justify-between mt-2">
        <template v-for="item in menuTypeList" :key="item.mode">
            <a-tooltip placement="bottom" :class="{'!cursor-not-allowed':(getConfigState('sysMode')==='phone')}">
                ...
                <div 
                    class="layout-item" 
                    :class="[item.mode,{'active':getConfigState('menuType') == item.mode}]"
                ...
                >
                    ...
                    <div class="icon"><check-circle-outlined :style="{color:getConfigState('themeColor')}" /></div>
                </div>
            </a-tooltip>
        </template>
    </div>
...
interface menuTypeInterface { title:string, mode:string }
// 菜单类型
const menuTypeList = ref<menuTypeInterface[]>([
   { title:'暗色左侧菜单',mode:menuTypeEnum.verticalDark },
   { title:'亮色左侧菜单',mode:menuTypeEnum.verticalLight },
   { title:'顶部菜单',mode:menuTypeEnum.horizontal },
   { title:'分割式菜单',mode:menuTypeEnum.mix }
])
const toggleMenu = (val:menuTypeInterface)=>{
    if(getConfigState('sysMode') === 'phone'){
        return message.error('H5暂不支持导航模式切换')
    }
...
.layout-item{
    ...
    display: flex;
    .layout-menu, .layout-light, .layout-content, .layout-container{
        border-radius: 3px;
    }
    .icon{
        display: block;
