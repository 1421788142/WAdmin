            ...
            <div class="max-h-[500px] overflow-y-auto">
                ...
                >
                    <div class="flex items-center duration-300 menu-item">
                        <component class="mr-2" v-if="item.length > 0 && item[0].icon" :is="item[0].icon"></component>
                        <template  v-if="item.length > 0">
                            <div v-for="(val,ind) in item" :key="ind">
                                <span>{{val.title}}</span> <span class="mx-2" v-if="ind < item.length-1">></span>
                            </div>
                        </template>
                    </div>
                    <enter-outlined />
...
const toPage = (val:Active[])=>{
    setHighlight(val)
    if(route.path === menuActive.value.path) return message.error('已在当前页面')
...
const onKeyDown = (e:KeyboardEvent)=>{
    ...
    if(isHasKey && menuList.value.length && menuActive.value && visible.value){
        ...
        if(downKey === 38){ //上
            if(index > 0){
                menuActive.value = menuList.value[index-1][menuList.value[index-1].length -1]
            } else {
