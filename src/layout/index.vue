<template>
    <div class="w-screen h-screen overflow-hidden bg-slate-500">
        ...
        <a-layout class="w-full h-full">
            <!-- 侧边栏 -->
            <a-layout-sider :collapsedWidth="50"
                ...
                <a-layout-header class="md:!pr-4 !px-1 !bg-white dark:!bg-[#141414] !h-max">
                    <div class="flex items-center justify-between" v-if="!getConfigState('isHasFull')">
                        <div class="flex items-center"
                            v-if="menuType === 'verticalDark' || menuType === 'verticalLight' || getConfigState('sysMode') === sysModeEnum.phone">
                            <div class="flex items-center" v-if="getConfigState('sysMode') === sysModeEnum.web">
                                <menu-unfold-outlined v-if="isHasCollapsed" class="text-xl"
                                    @click="setConfigState('isHasCollapsed', false)" />
                                <menu-fold-outlined v-else class="text-xl"
                                    @click="setConfigState('isHasCollapsed', true)" />
                                <breadcrumb />
                            </div>
                            <div class="flex items-center" v-if="getConfigState('sysMode') === sysModeEnum.phone">
                                <menu-unfold-outlined v-if="wapMenuVisible" class="text-xl"
                                    @click="wapMenuVisible = !wapMenuVisible" />
                                <menu-fold-outlined v-else class="text-xl" @click="wapMenuVisible = !wapMenuVisible" />
                            </div>
                        </div>
                        <div v-if="menuType === 'horizontal' && getConfigState('sysMode') === sysModeEnum.web"
                            class="flex flex-1">
                            <logo :collapsed="isHasCollapsed" :menuType="menuType" v-if="getConfigState('isHasLogo')" />
                            <menuVue menuTheme="light" />
                        </div>
                        <!-- 混合模式顶部菜单 -->
                        <div v-if="menuType === 'mix' && getConfigState('sysMode') === sysModeEnum.web"
                            class="flex flex-1 max-w-[80%]">
                            <a-menu mode="horizontal" class="!justify-center flex-1 max-w-[100%]"
