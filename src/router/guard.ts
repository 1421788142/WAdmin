...
class Guard {
    ...
    public run() {
        ...
        this.router.beforeEach(async (to: RouteLocationNormalized, from: RouteLocationNormalized) => {
            ...
            if (to.meta.hidden) return { name: 'noAuthority' } //判断是否有权限访问
    ...
    // 是否需要登录
    private isLogin(route: RouteLocationNormalized, userInfo: userInterface) {
        return Boolean(route.name !== 'login' && !userInfo)
    }
    // 已经登录禁止到登录页
    private prohibitLogin(route: RouteLocationNormalized, userInfo: userInterface) {
        return Boolean(route.name === 'login' && userInfo)
    }
}
